...
namespace WebCoreService
{
    ...
    public class Startup
    {
        ...
        {
            string excelCachePath = Path.Combine(Directory.GetCurrentDirectory(), @"\.Cache\ExportExcel");
            string iamgeCachePath = Path.Combine(Directory.GetCurrentDirectory(), @"\.Cache\Image");
            string localDBCachePath = Path.Combine(Directory.GetCurrentDirectory(), @"\.LocalDB");
            if (!Directory.Exists(excelCachePath))
            {
                Directory.CreateDirectory(excelCachePath);
            }

            if (!Directory.Exists(iamgeCachePath))
            {
                Directory.CreateDirectory(iamgeCachePath);
            }

            if (!Directory.Exists(localDBCachePath))
            {
                Directory.CreateDirectory(localDBCachePath);
            }
        }
        ...
        public Startup(IWebHostEnvironment env)
        {
            ...
            if (env != null)
            {
                IConfigurationBuilder builder = new ConfigurationBuilder()
                                    ...
                                    .AddJsonFile(Path.Combine(IOHelper.GetBinRunDir(), $@".Config\appsettings.{env.EnvironmentName}.json"), optional: true)
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                log.Error($"ConfigureServices Error:\r\n ${ex.ToString()}");
            }
                               ...
                               ILoggerFactory loggerFactory */ )
        {
            ...
            try
            {
                ...
                #region 确保数据库生成，并暖机
                CoreContextStaticMethod.WarmupAndEnsureDB();
                #endregion
            ...
            {
                string errorInfo = $"Startup.ConfigureE rror : {ex.Message} \r\n{ex.ToString()} ";
                log.Error(errorInfo);
